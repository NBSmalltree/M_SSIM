...
double l2r(Mat& lColor, Mat& rColor, Mat& lDisp, Mat& rDisp)
{
	...
	for (int i = 0; i < lGray.rows; i++) {
		uchar* data = lwarpedLocal_Value.ptr<uchar>(i);//行指针;
		for (int j = 0; j < lGray.cols; j++)
			data[j] = 0;
	}
	...
	//生成位置掩模;
	for (int i = 0; i < lGray.rows; i++) {
		uchar* data1 = lDisp.ptr<uchar>(i);//左视点视差图;
		ushort* data2 = lwarpedLocal.ptr<ushort>(i);//位置;
		uchar* data3 = lwarpedLocal_Value.ptr<uchar>(i);//映射过去的视差值;
		for (int j = 0, k = 0; j < lGray.cols; j++) {//k代表映射后的目标像素点;
			k = j - data1[j];
			if (k >= 0 && k < lGray.cols) {
				if (data3[k] < data1[j]) {//目标像素点存储的视差值<原始像素点视差值;
	...
	hrMask.create(lGray.rows, lGray.cols, CV_8UC1);//CV_8UC1
												   ...
												   //init to 0;
	for (int i = 0; i < lGray.rows; i++) {
		uchar* data = hrMask.ptr<uchar>(i);//行指针;
		for (int j = 0; j < lGray.cols; j++)
			data[j] = 0;
	}
	...
	for (int i = 0; i < lwarpedGray.rows; i++) {
		...
		for (int j = lwarpedGray.cols - 1; j >= 0; j--) {//从右开始遍历，遇到第一个非0中断循环;
			if (data1[j] == 0) {
				data2[j] = 1;
			}
			else
	...
	for (int i = 0; i < lGray.rows; i++) {
		...
		for (int j = 0; j < lGray.cols; j++) {//注意这里要除去Hr空间;
			if (data4[j] != 1) {
				data3[j] = abs(data1[j] - data2[j]);
			}
		}
	...
	//因数据类型关系，重新申明rGray,lwarpedGray,rMask(0~255),hrMask(0~1)为double类型数组（1342*1110）;
	int M = lGray.rows, N = lGray.cols;
	//因栈大小问题报错，故而转为二维指针方式的堆方法;
